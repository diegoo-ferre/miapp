...
while(rs.next()){
    ...
    String sueldoBaseTexto = guarani.format(rs.getDouble("sueldo_base")).replace(',', '.');
    String descuentoTexto = guarani.format(rs.getDouble("monto_descuento")).replace(',', '.');
    String sueldoFinalTexto = guarani.format(rs.getDouble("sueldo_final")).replace(',', '.');
%>
...
<tr>
    ...
    <td>Gs. <%= sueldoBaseTexto %></td>
    <td><%= rs.getDouble("porcentaje_descuento") %> %</td>
    <td><%= rs.getString("motivo_descuento") %></td>
    <td>Gs. <%= descuentoTexto %></td>
    <td>Gs. <%= sueldoFinalTexto %></td>
    <td><%= fecha != null ? formato.format(fecha) : "" %></td>
    <td>

        <a href="editar_salario_guardado.jsp?id=<%= rs.getInt("id") %>" class="btn btn-warning btn-editar">
            editar
        </a>
